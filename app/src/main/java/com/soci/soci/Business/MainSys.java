...
import com.soci.soci.Database.Event_Helper;
...
import com.soci.soci.Database.Person_Event_Participation_Table;
import com.soci.soci.Database.Person_Helper;
...
//        Person p = new Person(1, "Ahmet Oğuz", "Ergin", "ahmet@hotmail.com", "1234", "05052082324", "Male", participated_Events, created_Events);
//        people.add(p);
//
...
//        p = new Person(2, "Emre", "Cura", "emre@hotmail.com", "1234", "05051112233", "Male", participated_Events, created_Events);
//        people.add(p);
//
//        participated_Events = new ArrayList<>();
//        created_Events = new ArrayList<>();
...
//        p = new Person(3, "Bertan", "Özer", "bertan@hotmail.com", "1234", "05051112233", "Male", participated_Events, created_Events);
//        people.add(p);
//
//        participated_Events = new ArrayList<>();
//        created_Events = new ArrayList<>();
...
//    }
    ...
    public static void prepareDatabaseData(DatabaseHelper dbHelper) {
        ArrayList<Person_Helper> tempPersonList = Person_Table.getAll(dbHelper);
        ArrayList<Event_Helper> tempEventList = Event_Table.getAll(dbHelper);
        ArrayList<Person_Event_Helper> tempOwnerList = Person_Event_Owner_Table.getAll(dbHelper);
        ArrayList<Person_Event_Helper> tempParticipatedList = Person_Event_Participation_Table.getAll(dbHelper);

        events.clear();
        people.clear();
        Person p;
        ArrayList<Integer> oL;
        ArrayList<Integer> pL;
        Event e;


        for (Person_Helper tempPerson : tempPersonList) {
            oL = new ArrayList<>();
            pL = new ArrayList<>();

            for (Person_Event_Helper peh : tempOwnerList) {
                if (peh.getPerson_Id() == tempPerson.getId())
                    oL.add(peh.getEvent_Id());
            }

            for (Person_Event_Helper peh : tempParticipatedList) {
                if (peh.getPerson_Id() == tempPerson.getId())
                    pL.add(peh.getEvent_Id());
            }

            p = new Person(tempPerson.getId(), tempPerson.getName(), tempPerson.getSurname(), tempPerson.getEmail(), tempPerson.getPassword(), tempPerson.getPhone(), tempPerson.getGender(), pL, oL);

            people.add(p);
        }

        for (Event_Helper eh : tempEventList) {
            e = new Event(eh.getId(), eh.getMax_Participant(), eh.getName(), eh.getCategory(), eh.getDescription(), eh.getLocation(), eh.getStart_Date(), eh.getEnd_Date());
            events.add(e);
        }
    }
